...
def get_subscale_weights(texts: dict[str, str], excluded_subscales: list[str]) -> dict[str, float]:
	...
	input_embedding = model.encode(list(texts.values()), convert_to_tensor=True)
	scores = {}

	for disorder, phrases in filter(lambda kvp: kvp[0] not in excluded_subscales, psychiatric_templates.items()):
		phrases_embedding = model.encode(phrases, convert_to_tensor=True, normalize_embeddings=True)
		cosine_scores = util.cos_sim(input_embedding, phrases_embedding)
		max_per_input = cosine_scores.max(dim=1).values
		avg_score = max_per_input.mean().item()
